...
export default function ProductDetail() {
  ...
  if (loading) {
    return (
      <div className="flex flex-col items-center justify-center min-h-screen bg-white font-jakarta">
        ...
        <p className="text-[10px] font-black uppercase tracking-[0.4em] text-slate-400">Loading details...</p>
      </div>
    );
  }
  ...
  if (!product) {
    return (
      <div className="flex flex-col items-center justify-center min-h-screen text-center px-6 bg-white font-jakarta text-slate-900">
        <div className="h-20 w-20 bg-slate-50 rounded-full flex items-center justify-center mb-8">
           <ShoppingBag size={32} className="text-slate-200" />
        </div>
        <h2 className="text-2xl font-bold mb-4 uppercase tracking-widest">Product not found</h2>
        <Link to="/shop" className="inline-flex items-center gap-3 bg-blue-600 text-white h-12 px-8 rounded-xl font-bold text-xs uppercase tracking-widest hover:bg-blue-700 transition-all shadow-lg shadow-blue-600/20 active:scale-95">
          Return to gallery
        </Link>
      </div>
    );
  ...
  return (
    <div className="bg-white min-h-screen pt-32 pb-24 font-jakarta text-slate-900 overflow-x-hidden">
      <SEO title={`${product.name} | Axel Printing`} description={product.description?.substring(0, 160)} />
      
      <div className="max-w-[1200px] mx-auto px-4 md:px-6 lg:px-10 relative z-10">
        
        {/* --- BREADCRUMBS --- */}
        <nav className="flex items-center gap-2 text-[10px] font-bold uppercase tracking-widest text-slate-400 mb-12 overflow-hidden">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-12 lg:gap-16 items-start">
          ...
          <div className="lg:col-span-7 space-y-6">
            <div className="relative aspect-square bg-slate-50 rounded-3xl flex items-center justify-center p-8 md:p-16 overflow-hidden border border-slate-100 group transition-all duration-500 hover:border-blue-100">
              <motion.img 
                key={activeImage}
              ...
              <div className="absolute top-6 right-6 z-20">
                <button 
                  ...
                  className={cn(
                    "h-12 w-12 rounded-full transition-all duration-300 flex items-center justify-center active:scale-90 shadow-lg border border-slate-100",
                ...
                {images.map((img, idx) => (
                  <button 
                    ...
                    className={cn(
                      ...
                      activeImage === idx ? "border-blue-600 shadow-lg" : "border-slate-100 hover:border-slate-200"
                    )}
                  >
          ...
          <div className="lg:col-span-5 space-y-10 lg:pt-4">
            <div className="space-y-6">
              ...
              <div className="flex items-baseline gap-6">
                <span className="text-4xl font-black text-slate-900 tracking-tight">${parseFloat(product.price).toLocaleString()}</span>
                {product.sale_price && (
                  <span className="text-xl font-bold text-slate-300 line-through">${parseFloat(product.sale_price).toLocaleString()}</span>
                )}
              </div>
            </div>
            ...
            <div className="space-y-8 pt-8 border-t border-slate-100">
              <div className="flex flex-col sm:flex-row items-stretch gap-4">
                {/* Minimal Counter */}
                <div className="h-14 bg-slate-50 border border-slate-100 rounded-xl flex items-center justify-between px-2 w-full sm:w-[150px]">
                ...
                <button 
                  ...
                  disabled={isAdded}
                  className="flex-1 h-14 rounded-xl bg-blue-600 text-white font-bold uppercase text-[11px] tracking-widest shadow-xl shadow-blue-600/20 hover:bg-blue-700 active:scale-95 transition-all flex items-center justify-center gap-3 disabled:opacity-50"
                >
              ...
              <div className="grid grid-cols-3 gap-4 py-2">
                {[
                  ...
                  <div key={i} className="flex flex-col items-center gap-2 text-center group">
                    <div className="text-slate-300 group-hover:text-blue-600 transition-colors duration-300">{item.icon}</div>
          ...
          <div className="mt-32 pt-20 border-t border-slate-100">
            ...
            <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-6">
              {relatedProducts.slice(0, 5).map((p) => (
                <div key={p.id} className="group relative flex flex-col h-full bg-white transition-all duration-500">
                  <div className="relative aspect-square w-full bg-slate-50 overflow-hidden border border-slate-100 group-hover:border-blue-100 transition-all duration-500 rounded-2xl">
                    <img src={getImagePath(p.images)} className="w-full h-full object-contain p-6 mix-blend-multiply transition-transform duration-700 group-hover:scale-110" alt={p.name} />
                    <Link to={`/product/${p.slug}`} className="absolute inset-0 z-10" />
                  </div>

                  <div className="pt-4 px-1 space-y-1">
                    <Link to={`/product/${p.slug}`} className="block">
                      <h3 className="text-[13px] font-bold text-slate-800 line-clamp-1 group-hover:text-blue-600 transition-colors uppercase tracking-tight">{p.name}</h3>
                    </Link>
